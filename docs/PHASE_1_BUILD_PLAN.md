...
Build a production-ready ServiceNow scoped application that agent-to-agent (a2a) bridge facilitating secure, authenticated communication between autonomous ai agents running on different servicenow instances or external agent platforms.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
